...
The third columns of Tables 7-10 provide details of where people who where living in the inner region one and five years before the 2016 and 2021^[i.e. in 2011, 2015, 2016 and 2020] had moved to in 2016 and 2021, respectively. Table 11 collects this data.
...
moving_from_inner <- cbind(X2016_combined_1yr_ago[,1], X2016_combined_5yr_ago[,3], X2016_combined_1yr_ago[3], X2021_combined_5yr_ago[,3], X2021_combined_1yr_ago[,3])
names(moving_from_inner) <- c("Destination", "Moving 2011-16", "Moving 2015-16", "Moving 2016-21", "Moving 2020-21")


#output table with percentages etc. 
knitr::kable(
 moving_from_inner %>% adorn_totals("row") %>% adorn_percentages("col") %>% adorn_pct_formatting() %>% adorn_ns(ns = format(moving_from_inner %>% adorn_totals("row"), big.mark=","), position = "front"), row.names = FALSE, format.args = list(big.mark = ",", scientific = FALSE), align = "lrrrr", caption = 'Table 11: Destinations of those moving FROM the Inner region to locations within Victoria prior to the census')
...
moving_from_inner <- rbind(moving_from_inner, moving_from_inner_rest_of_Australia)
...
NEXT STEPS:
1. Statistical tests comparing 2011-16 to 2016-21 
2. Statistical tests comparing 2015-16 to 2020-21 
3. Then renumber tables from here onwards
